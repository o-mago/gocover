...
func TestInExclueds(t *testing.T) {
	t.Run("inExclueds", func(t *testing.T) {
		...
		testSuites := []struct {
			...
			{input: "/mock_client/dbclient.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/mock_client/interface.go", expect: true}, // first match
			{input: "github.com/o-mago/gocover/pkg/mock_client/interface.go", expect: true}, // second match, hit cache
			{input: "github.com/o-mago/gocover/pkg/mock_client/dbclient.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/mock_client.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/mock.go", expect: true},
			{input: "zz_generated.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/api/v1/zz_generated.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/api/v1/zz_generated.deepcopy.go", expect: true},
			{input: "gocover.pb.go", expect: true},
			{input: "github.com/o-mago/gocover/protos/v1/gocover.pb.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/foo", expect: true},
			{input: "github.com/o-mago/gocover/pkg/foo/foo.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/foo/a/foo.go", expect: true},
			{input: "github.com/o-mago/gocover/pkg/client/dbclient.go", expect: false},
		}

		for _, testCase := range testSuites {
			acutal := inExclueds(cache, []string{"**/mock_*/**", "**/zz_generated*.go", "**/*.pb.go", "**/mock*.go", "github.com/o-mago/gocover/pkg/foo/**"}, testCase.input, logger)
...
func TestFormatFilePath(t *testing.T) {
	t.Run("formatFilePath", func(t *testing.T) {
		testSuites := []struct {
			...
			{input: "/home/user/go/src/Azure/gocover/pkg/foo/foo.go", expect: "github.com/o-mago/gocover/pkg/foo/foo.go"},
			{input: "/src/pkg/foo/foo.go", expect: "github.com/o-mago/gocover/src/pkg/foo/foo.go"},
		}

		for _, testCase := range testSuites {
			acutal := formatFilePath("/home/user/go/src/Azure/gocover", testCase.input, "github.com/o-mago/gocover")
			// only care about linux os
			if runtime.GOOS == "linux" {
				if acutal != testCase.expect {
...
func TestReBuildStatistics(t *testing.T) {
	t.Run("reBuildStatistics", func(t *testing.T) {
		...
		cache := excludeFileCache{"github.com/o-mago/gocover/pkg/foo/foo.go": true}
...
func TestParseGoModulePath(t *testing.T) {
	t.Run("parse go module path from go.mod", func(t *testing.T) {
		...
		ioutil.WriteFile(filepath.Join(dir, "go.mod"), []byte("module github.com/o-mago/gocover"), 0644)
		ioutil.WriteFile(filepath.Join(dir, "foo/go.mod"), []byte("module github.com/o-mago/gocover/foo"), 0644)
		...
		var testSuites = []struct {
			...
			{input: filepath.Join(dir, "."), expect: "github.com/o-mago/gocover", err: nil},
			{input: filepath.Join(dir, "foo"), expect: "github.com/o-mago/gocover/foo", err: nil},
